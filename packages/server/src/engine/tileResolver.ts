...
import { addToJackpot } from './jackpotSystem';
...
export function resolveTileLanding(
  ...
  const result: TileResolutionResult = {
    playerUpdates: {},
    jackpotIncrease: 0,
    tileEvents: [],
    goToJail: false,
  ...
  switch (tile.type) {
    ...
    case 'tax': {
      const taxTile = tile as TaxTile;
      result.pendingAction = { type: 'pay_tax', amount: taxTile.amount };
      result.tileEvents.push(
        `${player.name} muss Steuer zahlen: ${taxTile.amount}€`
      );
      break;
    }
    ...
    case 'utility': {
      ...
      if (!purchasable.ownerId) {
        ...
        result.tileEvents.push(
          `${player.name} landet auf ${tile.name} (unbesessen)`
        );
        ...
        if (tile.type === 'property' && !purchasable.mortgaged) {
          ...
          if (prop.houses < 5) {
            ...
            result.tileEvents.push(
              `${player.name} landet auf eigener Immobilie ${tile.name} - Haus bauen möglich`
            );
          } else {
            result.tileEvents.push(
              `${player.name} landet auf eigener Immobilie ${tile.name} (Hotel bereits vorhanden)`
            );
          }
        } else {
          result.tileEvents.push(
            `${player.name} landet auf eigener Immobilie ${tile.name}`
          );
        }
      } else {
        // Owned by another player - pay rent
        const rentResult = calculateRent(tile, player, allTiles, diceTotal);
        if (rentResult && !purchasable.mortgaged) {
          ...
          result.tileEvents.push(
            `${player.name} muss Miete zahlen: ${rentResult.rentAmount}€`
          );
        } else {
          result.tileEvents.push(
            `${player.name} landet auf ${tile.name} (verpfändet - keine Miete)`
          );
        }
      }
      break;
