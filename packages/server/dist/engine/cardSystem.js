...
function createDeck(deckType) {
    const templates = deckType === 'chance' ? cards_config_1.CHANCE_CARDS : cards_config_1.COMMUNITY_CHEST_CARDS;
    const deck = templates.map((card) => ({
        ...card,
        id: (0, uuid_1.v4)(),
    }));
 ...
 */
function applyCardEffect(card, player, allPlayers, tiles, config) {
    ...
    switch (card.type) {
        ...
        case 'collect_from_players': {
            const amount = card.amount ?? 0;
            const otherPlayers = updatedPlayers.filter((p) => p.id !== player.id && !p.isBankrupt);
            ...
            updatedPlayers = updatedPlayers.map((p) => p.id === player.id ? { ...updatedPlayer } : p);
            message = `${card.text} (+${totalCollected}€)`;
            break;
        }
        case 'move_to': {
            const targetPos = card.position ?? 0;
            const isGoCard = card.deckType === 'community_chest' && targetPos === 0;
            if (isGoCard) {
                // Community chest "Go to GO" → exact landing (400€)
                const result = (0, movement_1.movePlayerTo)(updatedPlayer, targetPos, config, true // exact landing
                );
                ...
                const result = (0, movement_1.movePlayerTo)(updatedPlayer, targetPos, config, false);
                updatedPlayer = result.player;
                newPosition = result.movement.newPosition;
                passedGo = result.movement.passedGo;
                landedOnGo = result.movement.landedOnGo;
        ...
        case 'move_back': {
            ...
            const pos = (0, movement_1.moveBack)(updatedPlayer.position, spaces);
        ...
        case 'go_to_jail': {
            updatedPlayer = (0, jailSystem_1.sendToJail)(updatedPlayer, config.jailPosition);
            goToJail = true;
        ...
        case 'move_to_nearest_railroad': {
            const nearestRR = (0, movement_1.findNearestRailroad)(updatedPlayer.position);
            const result = (0, movement_1.movePlayerTo)(updatedPlayer, nearestRR, config, false);
            updatedPlayer = result.player;
            newPosition = result.movement.newPosition;
            passedGo = result.movement.passedGo;
            landedOnGo = result.movement.landedOnGo;
